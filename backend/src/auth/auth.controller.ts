...
export class AuthController {
  ...
  async signUp(@Body() body: any) {
    ...
    try {
      await this.db.execute(`INSERT INTO "user" (id,name,email,email_verified,created_at,updated_at) VALUES ('${id}','${name.replace(/'/g,"''")}','${email}',1,'${now}','${now}')` as any);
      const accId = `acc_${id}`;
  ...
  async signIn(@Body() body: any, @Res({ passthrough: true }) res: any) {
    const email = String(body.email || '').toLowerCase().trim();
    const password = String(body.password || '');
    const r: any = await this.db.execute(`SELECT u.id, u.name, u.email, a.password FROM "user" u JOIN account a ON a.user_id=u.id WHERE lower(u.email)='${email.replace(/'/g,"''")}' LIMIT 1` as any);
    ...
    return { user: { id: row.id, name: row.name, email: row.email }, token };
  }
