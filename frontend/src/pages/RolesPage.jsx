...
export default function RolesPage({ bare = false }) {
  ...
  async function deleteUser(id) {
    ...
    const ok = await confirm({
      title: `Hapus "${u.name}"?`,
      description: 'Akun dihapus dari daftar dan tidak bisa login lagi.',
      variant: 'danger',
      confirmLabel: 'Ya, Hapus',
    });
    if (!ok || konfirmBusy) return;
    setKonfirmBusy(true);
    try {
  ...
  async function removeConn(id) {
    ...
    if (!hit) return;
    const ok = await confirm({
      title: `Hapus "${hit.name}"?`,
      description: 'Koneksi AI dihapus dari daftar.',
      variant: 'danger',
      confirmLabel: 'Ya, Hapus',
    });
    if (!ok) return;
  ...
  return (
    <div className={bare ? 'space-y-6' : 'page'}>
      ...
      <Reveal className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Pengguna" value={stats.users} sub={`${stats.active} aktif`} />
        <StatCard title="Jumlah Role" value={ROLES.length} sub="Super Admin s/d Viewer" />
        <StatCard title="Modul Sistem" value={MODULES.length} sub="Dengan izin terpisah" />
        <StatCard title="Nonaktif" value={stats.inactive} sub="Akun ditangguhkan" valueCls="text-rose-600" subCls="text-rose-500" />
      </Reveal>

      {/* Tab */}
      <div className="flex items-center gap-2 bg-slate-200/60 dark:bg-slate-800 rounded-xl p-1 w-fit max-w-full overflow-x-auto scrollbar-thin text-sm font-semibold">
        {[
          ...
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`shrink-0 px-4 py-2 min-h-[44px] inline-flex items-center rounded-lg transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/60 ${tab === t.id ? 'bg-white dark:bg-slate-700 text-brand-700 dark:text-brand-300 shadow-sm' : 'text-slate-500 dark:text-slate-300'}`}
          >
            {t.label}
          </button>
        ))}
        ...
        <Reveal className="space-y-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 flex flex-wrap items-center gap-3">
            <div className="relative flex-1 min-w-[220px]">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2.5 min-h-[44px] text-sm border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500/40 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 placeholder:text-slate-500 dark:placeholder:text-slate-400"
              />
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
            ...
            <table className="w-full text-sm min-w-[720px]">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800/70 text-left text-[11px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 border-b border-slate-200 dark:border-slate-700">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          aria-label={`${u.active ? 'Nonaktifkan' : 'Aktifkan'} akun ${u.name}`}
                          title={u.active ? 'Nonaktifkan' : 'Aktifkan'}
                          className={`relative w-10 h-[22px] rounded-full transition before:absolute before:-inset-3 before:content-[""] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/60 ${u.active ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-700'}`}
                        >
                          <span
                            className={`absolute top-[3px] w-4 h-4 rounded-full bg-white shadow transition-all ${u.active ? 'left-[21px]' : 'left-[3px]'}`}
                          />
                        </button>
                        <span className={`text-xs font-semibold ${u.active ? 'text-emerald-600' : 'text-slate-500 dark:text-slate-400'}`}>
            ...
            {filteredUsers.length === 0 && (
              <div className="px-6 py-12 text-center text-sm text-slate-500 dark:text-slate-400">Tidak ada pengguna yang cocok dengan filter.</div>
            )}
            </div>
          </div>
        </Reveal>
        ...
        <Reveal className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
            {roleCards.map(({ r, i, count, modCount }) => (
              <div key={r} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4">
                <span className={`inline-block text-[11px] font-bold border rounded-full px-2.5 py-1 ${ROLE_STYLE[r]}`}>{r}</span>
                <p className="mt-2.5 text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed min-h-[32px]">{ROLE_DESC[r]}</p>
                <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs">
                  <span className="font-bold text-slate-700 dark:text-slate-200">{count} pengguna</span>
                  <span className="text-slate-500 dark:text-slate-400">{modCount}/{MODULES.length} modul</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
            <div className="px-4 sm:px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between flex-wrap gap-3">
              <div>
          ...
          className="inline-flex items-center gap-2 bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold px-4 py-2.5 min-h-[44px] rounded-lg shadow-md shadow-brand-600/25 transition active:scale-95"
            ...
            <div className="overflow-x-auto scrollbar-thin">
              <table className="w-full text-sm min-w-[680px]">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800/70 text-[11px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 border-b border-slate-200 dark:border-slate-700">
                        ...
                        return (
                          <td key={r} className="px-4 py-3.5 text-center">
                            <input
                              ...
                              aria-label={`Izin ${m.name} untuk role ${r}`}
                              className="w-6 h-6 text-brand-600 rounded-md border-slate-300 focus:ring-brand-500 disabled:opacity-60 cursor-pointer"
                            />
                          </td>
                        );
                      })}
            ...
            <p className="px-4 sm:px-6 py-3 text-[11px] text-slate-500 dark:text-slate-400 border-t border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/40">
      ...
      {tab === 'ai' && (
        <Reveal className="space-y-4">
            ...
            <div key={c.id} className={`bg-white dark:bg-slate-900 rounded-2xl border p-4 sm:p-5 ${c.active ? 'border-emerald-300 dark:border-emerald-500/40 ring-1 ring-emerald-200 dark:ring-emerald-500/20' : 'border-slate-200 dark:border-slate-800'}`}>
              <div className="flex items-center gap-3 flex-wrap">
                <button
                  ...
                  aria-label={`${c.active ? 'Nonaktifkan' : 'Pakai'} ${c.name}`}
                  title={c.active ? 'Nonaktifkan (chat jadi mode lokal)' : 'Pakai AI ini'}
                  className={`relative w-12 h-7 rounded-full transition shrink-0 before:absolute before:-inset-3 before:content-[""] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/60 ${c.active ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-700'}`}
                >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 font-mono">{c.provider} · {c.model}</p>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate">{c.baseURL}</p>
                </div>
          ...
          {conns.length === 0 && !showAiForm && (
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700 p-10 text-center text-sm text-slate-500 dark:text-slate-400">
              Belum ada AI terdaftar — klik <span className="font-bold">+ Tambah AI</span>.
            </div>
          )}

          {showAiForm && (
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-brand-200 dark:border-brand-500/20 p-4 sm:p-6">
              ...
              {logs.map((l, i) => (
                <li key={i} className="ml-5 relative">
                  ...
                  <p className="text-sm text-slate-700 dark:text-slate-200">
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">{fmtLogTime(l.time)}</p>
                </li>
              ))}
              </ol>
          </div>
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="mt-1 w-full min-h-[44px] text-sm border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand-500/40 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 placeholder:text-slate-500 dark:placeholder:text-slate-400"
                />
              </div>
              <div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`relative w-10 h-[22px] rounded-full transition before:absolute before:-inset-3 before:content-[""] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/60 ${modal.active ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-700'}`}
                >
                  <span
...
function StatCard({ title, value, sub, valueCls = '', subCls = 'text-slate-500 dark:text-slate-400' }) {
  return (
