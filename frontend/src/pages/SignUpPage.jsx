...
const inputCls = (invalid) =>
  `w-full min-h-[44px] px-3 py-3 text-sm border rounded-lg focus:outline-none focus:ring-2 transition bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 placeholder:text-slate-500 dark:placeholder:text-slate-400 ${
    invalid
      ...
      <div className="w-full max-w-4xl motion-safe:animate-fade-in">
        <div className="grid md:grid-cols-[320px_1fr] bg-white dark:bg-slate-900 rounded-2xl shadow-xl shadow-brand-600/10 border border-slate-200 dark:border-slate-800 overflow-hidden">
          ...
          <aside className="bg-gradient-to-b from-brand-700 to-brand-950 text-white p-7 flex flex-col gap-6">
            ...
            {/* R-31: stepper vertikal disembunyikan di HP agar panel konteks ringkas;
                progres tetap terbaca via progressbar + "Langkah X dari 3" di panel form. */}
            <ol className="space-y-1 hidden md:block" aria-label="Langkah pendaftaran">
              {STEPS.map((s, i) => {
                  ...
                  <div>
                    ...
                    {values.password && (
                      <div id="reg-password-strength" className="mt-2" aria-live="polite">
                        ...
                        <p className="text-[11px] text-slate-500 mt-1">Kekuatan: <span className="font-semibold text-slate-600 dark:text-slate-300">{PASSWORD_STRENGTH_LABEL[strength]}</span></p>
                      </div>
                    )}
                  </div>
                  <div>
