...
// file: { filename, sizeLabel } (mode file/PDF) | busy: hapus sedang berjalan
// | onCancel() | onConfirm()
// Mode generik: title + message + itemLabel/itemSub (tanpa kotak file).
// Tanpa prop baru, render identik seperti semula (kompatibel mundur).
export default function DeleteConfirmModal({ file, busy = false, onCancel, onConfirm, title, message, itemLabel, itemSub }) {
  const judul = title || 'Hapus PDF?';
  const pesan = message || 'File berikut akan dihapus permanen dari penyimpanan dan tidak bisa dikembalikan:';
      ...
      <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-fade-in-fast">
        <div className="bg-gradient-to-r from-rose-600 to-red-500 px-5 pt-5 pb-4 text-white">
          <div className="flex items-center gap-3">
            ...
            <h3 id="delete-confirm-title" className="text-lg font-extrabold">{judul}</h3>
          </div>
        </div>
        <div className="px-5 py-4">
          ...
          <div className="mt-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/70 px-3 py-2.5">
            <p className="truncate text-[12px] font-bold text-slate-700 dark:text-slate-200" title={itemLabel}>{itemLabel}</p>
            {itemSub && <p className="mt-0.5 text-[11px] font-semibold text-slate-500 dark:text-slate-400">{itemSub}</p>}
          </div>
          ) : null}
